...
#' @param pcs A vector of length 2, specifying which principal components should be plotted
...
plotAlignedSamples <- function(preds, pcs=c(1,2), color_by="SRS") {
  ...
  if(color_by == "SRS") {
    return(
      ggplot2::ggplot(pca_coords[pca_coords$Study != "Input", ], ggplot2::aes(x=eval(parse(text=paste("PC",pcs[1],sep=""))), y=eval(parse(text=paste("PC",pcs[2],sep=""))))) +
        ...
        ggplot2::xlab(paste("PC", pcs[1], ": ", pca_var[pcs[1]], "% of variance explained", sep="")) +
        ggplot2::ylab(paste("PC", pcs[2], ": ", pca_var[pcs[2]], "% of variance explained", sep="")) +
